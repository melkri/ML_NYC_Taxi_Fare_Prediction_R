...
calculate_mi <- function(x, y) {
  mi_score <- entropy(cbind(x, y))
  return(mi_score)
}

# Function to calculate correlation
calculate_correlation <- function(x, y) {
  correlation_matrix <- rcorr(cbind(x, y))
  correlation_value <- correlation_matrix$r[1, 2]
  return(correlation_value)
}

# Select numerical columns (excluding the target variable)
numerical_columns <- df_train %>% select_if(is.numeric) %>% select(-fare_amount)

# Create a table with mi_score, correlation for each variable
mi_correlation_table <- numerical_columns %>%
  summarise(across(everything(), list(
    mi_score = ~calculate_mi(., df_train$fare_amount),
    correlation = ~calculate_correlation(., df_train$fare_amount)
  ), .names = "{.col}_{.fn}"))

# Print the table
print(mi_correlation_table)
