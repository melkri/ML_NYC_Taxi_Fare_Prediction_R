...
We base on the dataset provided by the Lecturer, that consists of 17 independent variables and 1 dependent variable called: fare_amount. Out of 17 prepared variables there are also 10 so called "black box" variable that lack specific interpret ability during the model, however may be impactful on the final prediction.
...
As all of the non-meaningful 0 values are in longitude/latitude columns, we have decided to apply binary value for whether we have meaningful geospatial values or not, and in case of existing values, calculate the straight distance between those two points taking into account the curvature of the earth.
...
As the early cleaning is over we can train/test split and save the data using 30% test size.
...
par(mfrow=c(5, 3))
numeric_variables <- names(df_train)[sapply(df_train, is.numeric) & !(names(df_train) %in% c("id", "pickup_date", "pickup_time", 'dropoff_latitude', 'dropoff_longitude', 'pickup_latitude','pickup_longitude'))]
...
numeric_variables <- names(df_train)[sapply(df_train, is.numeric) & !(names(df_train) %in% c("id", "pickup_date", "pickup_time", 'dropoff_latitude', 'dropoff_longitude', 'pickup_latitude','pickup_longitude'))]
for (var in numeric_variables) {
    plot(df_train[[var]], df_train$fare_amount, main=paste("Scatter plot of", var, "vs fare_amount"), xlab=var, ylab="fare_amount")
}
```
...
### Time dependency of fare variable
...
models <- list(
  lgbm = list(
    ...
    grid_1 = set_seed_grid(grid_latin_hypercube(learn_rate(), size = 8), 123),
    grid_2 = set_seed_grid(grid_latin_hypercube(tree_depth(), loss_reduction(), stop_iter(), size = 8), 123)
  ),
  xgboost = list(
    model_name = 'xgboost',
  ...
  decision_tree = list(
    ...
    rec_spec = recipe(fare_amount ~ ., df_train),
    grid_1 = set_seed_grid(grid_latin_hypercube(cost_complexity(), size = 8), 123),
    grid_2 = set_seed_grid(grid_latin_hypercube(tree_depth(), min_n(), size = 8), 123)
  ),
  random_forest = list(
    ...
    rec_spec = recipe(fare_amount ~ ., df_train),
    grid_1 = set_seed_grid(grid_latin_hypercube(trees(), size = 3), 123),
    grid_2 = set_seed_grid(grid_latin_hypercube(min_n(), size = 3), 123)
  )
)
...
combined_table <- bind_rows(lgbm_df, xgboost_df, decision_tree_df, random_forest_df)

datatable(combined_table, options = list(
  searching = FALSE,
  pageLength = 12,
  lengthMenu = c(5, 10, 15, 20),
  scrollX = TRUE
...
Based on the upper table we can notice extremely unusal prediction of xgboost models, with extremely high MAPE value in comparison to the other models. Because of that we have decided to use best performing configuration of random forest, decision tree and light gbm model.


## Predicting models on test dataset

``` {r, echo = FALSE}
rf_params <- list(trees = 1404, min_n = 20, mtry = 6)
dt_params <- list(tree_depth = 12, min_n = 32, cp = 0.0000632)
lgbm_params <- list(learn_rate = 0.01, tree_depth = 8, loss_reduction = 0.406, stop_iter = 12)
```

Let's also apply feature engineering do test dataframe as we did with training set.
...
df_test <- df_test %>%
  ...
  summarize(count = n(), avg_fare = mean(fare_amount)) %>%
  arrange(tenmin_interval) %>%
  left_join(df_test, by = "tenmin_interval") 
...
As we will use best performing models from random forest, lightGBM and decision tree in order to predict the taxi fare for test dataset and compare their MAPE metrics.

### Light GBM
```{r, cache = TRUE}
lgbm_model <- boost_tree(
  learn_rate = lgbm_params$learn_rate,
  tree_depth = lgbm_params$tree_depth,
  loss_reduction = lgbm_params$loss_reduction,
  stop_iter = lgbm_params$stop_iter
) %>%
  set_mode("regression") %>%
  set_engine("lightgbm")

# Create a workflow for the LightGBM model
wf_lgbm <- workflow() %>%
  add_model(lgbm_model) %>%
  add_formula(fare_amount ~ .)

# Train the LightGBM model on df_train
wf_lgbm_fit <- wf_lgbm %>% fit(data = df_train)

# Predict on df_test
lgbm_preds <- predict(wf_lgbm_fit, new_data = df_test)

# Calculate MAPE metric for test
mape_lgbm_test <- mean(abs(df_test$fare_amount - lgbm_preds$.pred) / df_test$fare_amount) * 100

# Calculate MAPE metric for train
lgbm_train_preds <- predict(wf_lgbm_fit, new_data = df_train)
mape_lgbm_train <- mean(abs(df_train$fare_amount - lgbm_train_preds$.pred) / df_train$fare_amount) * 100

# Create an iteration dataframe for LightGBM
lgbm_result <- data.frame(
  Mape_test = mape_lgbm_test,
  Mape_train = mape_lgbm_train,
  model_name = "lightgbm"
)
```
...
dt_model <- decision_tree(
  tree_depth = dt_params$tree_depth,
  min_n = dt_params$min_n,
  cost_complexity = dt_params$cp
) %>%
  set_mode("regression") %>%
  set_engine("rpart")
...
wf_dt_fit <- wf_dt %>% fit(data = df_train)

dt_preds <- predict(wf_dt_fit, new_data = df_test)

mape_dt_test <- mean(abs(df_test$fare_amount - dt_preds$.pred) / df_test$fare_amount) * 100

dt_train_preds <- predict(wf_dt_fit, new_data = df_train)
mape_dt_train <- mean(abs(df_train$fare_amount - dt_train_preds$.pred) / df_train$fare_amount) * 100

dt_result <- data.frame(
  Mape_test = mape_dt_test,
  Mape_train = mape_dt_train,
  model_name = "decision_tree"
)
...
wf_rf_fit <- wf_rf %>% fit(data = df_train)

rf_preds <- predict(wf_rf_fit, new_data = df_test)

mape_rf_test <- mean(abs(df_test$fare_amount - rf_preds$.pred) / df_test$fare_amount) * 100

rf_train_preds <- predict(wf_rf_fit, new_data = df_train)

mape_rf_train <- mean(abs(df_train$fare_amount - rf_train_preds$.pred) / df_train$fare_amount) * 100

rf_result <- data.frame(
  Mape_test = mape_rf_test,
  Mape_train = mape_rf_train,
  model_name = "random_forest"
)
```
...
``` {r, echo = FALSE}
combined_results <- bind_rows(dt_result, rf_result, lgbm_result)
combined_results$Model_Spec <- c(
  paste("trees =", rf_params$trees, ", min_n =", rf_params$min_n, ", mtry =", rf_params$mtry),
  paste("tree_depth =", dt_params$tree_depth, ", min_n =", dt_params$min_n, ", cp =", dt_params$cp),
  paste("learn_rate =", lgbm_params$learn_rate, ", tree_depth =", lgbm_params$tree_depth, ", loss_reduction =", lgbm_params$loss_reduction, ", stop_iter =", lgbm_params$stop_iter)
)
datatable(combined_results, options = list(
  searching = FALSE,
  pageLength = 3,
  lengthMenu = c(5, 10, 15, 20),
  scrollX = TRUE
...
To sum up, we build several ML models using different approaches. At the end the best result has been obtained by Random Forest [less than 10 MAPE score], however as we could also see Decision Tree has been extremely close in terms of the final prediction, with MAPE score little above 10. Light GBM has been a little bit behind oscilating at around 16 MAPE score. Although random forest having the highest predictive power, the computing complexity of fitting the model works in favor of Decision Tree.

In order to compute from start to finish the random forest, it took around 2 hours of calculation, however decision tree has finished under 30 seconds. 

As for usage of tidymodels, it has been rather intuitive throughout the whole project, comparing the feeling and simplicity of usage that we can experience from using scikit-learn in python.